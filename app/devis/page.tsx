        ...
        <section className="flex flex-col w-full bg-transparent items-center  pb-24 px-4 ">
            ...
            <form onSubmit={sendEmail} className="bg-white rounded-lg shadow-md p-8 w-full max-w-lg flex flex-col gap-6">
                ...
                */}
                ...
                <input type="tel" onChange={(e)=>{setFormData((prev)=>({...prev,telephone:e.target.value.toString()}));}} name="telephone" placeholder="Téléphone" className="border border-[#DCA797] rounded px-4 py-2 focus:outline-none focus:ring-2 focus:ring-[#DCA797]" required />
                        ...
                        services.map((service) => (
                            <div key={service.id} className="flex flex-row gap-2 cursor-pointer col-span-1">
                                <
                                    ...
                                    checked={selectedServices.includes(service.name.toString())}
                                    name={service.name} id={service.id.toString()}
                                    onChange={

                                        (e) => handleServiceToggle(e)
                                    }
                                />
                ...
                <textarea required name="message" value={formData.message} onChange={(e) => setFormData({ ...formData, message: e.target.value.toString() })} placeholder="Decrivez votre demande" rows={4} className="border border-[#DCA797] rounded px-4 py-2 focus:outline-none focus:ring-2 focus:ring-[#DCA797]" />
